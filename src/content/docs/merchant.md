...
> **快速重點**
> 商人（Merchant）的核心價值是 inventory upgrade：背包越大，越能帶補給，也越能把高價戰利品帶回城。
> 來源摘要提到背包可從 8 格逐步擴到 40 格，且每個角色都要各自支付升級費用。
> 商人（Merchant）也負責 leather armor、隨機裝備與定期 restock，是刷寶之外的補裝來源。
...
來源摘要把背包擴張視為商人（Merchant）最重要的服務。玩家初始背包空間很小，資料中整理為 8 格；透過名為 `Twiddles` 的商人支付金幣升級後，可逐步擴展，摘要提到上限可到 40 格。

背包升級的重點：

- 費用會逐次增加，不是一次買斷。
- 背包擴張是角色獨立的，每個角色都要各自升級。
- 背包越大，越能帶回未鑑定裝備、高價底材、藥水與任務途中掉落。
- 背包升級通常和技能學習、補給、鑑定成本競爭同一筆金幣。

> **提醒**
> 來源摘要提到背包升級入口通常在商人（Merchant）視窗左下角，且商人名稱為 `Twiddles`。UI 位置與名稱可能因版本或語系不同，找不到時先看目前商店視窗提示。

## 什麼時候該先升背包

背包不是純舒適性功能，它會直接改變地城收益。若你常在 boss 前就滿包，或不得不丟掉 yellow / blue 未鑑定裝備，背包升級的收益會很快浮現。

優先升背包的情境：

1. 每場地城都會滿包，導致你無法帶回高價裝備。
2. 你開始想保留 set、unique、抗性裝或分身裝。
3. 你需要多帶藥水、卷軸或資源補給。
4. 你正在 farm 特定部位，想提高每趟可帶回的候選數量。
5. 技能主循環已經能順利打怪，短期傷害提升不如多帶戰利品重要。
...
## 皮甲（Leather Armor）與隨機商品

商人（Merchant）也是 `Leather Armor` 的主要供應點。對德魯伊（Druid）、武僧（Monk）、天騎士（Templar）、巫師（Wizard）、術士（Warlock）這類最高穿到 leather 的職業來說，商店刷新有時能提供不錯的過渡裝。

看商人（Merchant）裝備時，優先確認：

- 等級是否貼近目前角色。
- 是否符合職業 armor tier 或 weapon proficiency。
- 詞綴是否補到抗性、生命、資源、主屬性、haste 或 talents。
- 價格是否合理，還是把金幣留給技能、背包與補給更划算。

來源摘要與公開 loot 資料都提醒，merchants 會依角色等級提供裝備，且會定期 restock。摘要提到庫存約每 1 小時的角色在線時間刷新；實際週期仍以目前版本為準。這讓商店巡視變成刷寶之外的小型抽卡：多半只是過渡品，但偶爾會遇到剛好補洞的好裝。
...
總結來說，商人（Merchant）是 NGO 的後勤槓桿。它賣的裝備未必永遠最強，但背包擴張會讓所有職業的刷寶效率變好；懂得把金幣分配在技能、背包與補給之間，是前期經營最重要的節奏感。
