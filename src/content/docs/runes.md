...
如果要做一套給分身輪流穿的經驗值升級裝，核心不是追單件裝備名稱，而是找「右手 / 左手 / 副手」三欄中可穿、sockets 夠多、等級需求合理的 weapon / `Charm` base，再放入 `Thex`。遊戲內 `Thex Rune` tooltip 顯示，單手武器、bow 與 `Charm`（護石 / 查姆）是 `+10% 經驗值取得率`，雙手武器是 `+20% 經驗值取得率`，但盾牌套用的是防具效果 `+15% 最大法力`。因此盾牌不能當經驗值加成來源；實際加成仍要以目前裝備 tooltip / 角色面板為準。

| 欄位 | 可用裝備 | 單顆 `Thex` | 6-slot 加成 | 職業限制 |
| ------ | ------ | ------ | ------ | ------ |
| 右手 / 左手 | 單手鈍器（`1h Blunt`） | `+10%` XP | 每把 `+60%` | 14 職業都可用；雙持後可放兩把 |
| 右手 / 左手 | 雙手鈍器（`2h Blunt`）或 `Stave` | `+20%` XP | 每把 `+120%` | 吟遊詩人與盜賊不可用；戰士可雙持雙手武器時另成極限路線 |
| 副手 / sub | Bow，特別是高 roll `Demetrium's Ballista` | `+10%` XP | `+60%` | 只限遊俠、戰士、十字軍、暗影騎士、盜賊、吟遊詩人 |
| 副手 / sub | `Charm`（護石 / 查姆） | `+10%` XP | 5 slots 先按 `+50%` | 14 職業都可用；不能用 bow 的職業尤其重要 |
| 左手 / 防具欄位 | 盾牌 | 無 XP | 無 XP | 盾牌可補生存，但 `Thex` 只給 `+15% 最大法力` |

滿洞套裝的成本可以直接這樣算：

| 套裝形態 | `Thex` 顆數 | 經驗值加成 | 讀法 |
| ------ | ------ | ------ | ------ |
| `1h + 1h` | 12 | `+120%` | 全職業最通用，但需要雙持 |
| `1h + 1h + bow` | 18 | `+180%` | Archery 六職的泛用高洞路線 |
| `2h` | 6 | `+120%` | Thex 用量最省的單件高倍率工具 |
| `2h + bow` | 12 | `+180%` | 可用 `2h Blunt` 且可用 bow 的職業才成立 |
| 戰士 `2h + 2h` | 12 | `+240%` | 戰士雙持雙手武器特例 |
| 戰士 `2h + 2h + bow` | 18 | `+300%` | 同樣是 18 顆 Thex，差別在雙手武器每洞倍率較高 |
| `1h + 1h + 5-slot Charm` | 17 | `+170%` | 不能用 bow 或想用 Charm 當 stat stick 時的保守算法 |

低等 6-socket `2h Blunt` / `Stave` 很適合當分身傳家工具，但要寫成「高泛用」而不是「全職業」。目前站內職業表列出的 `2h Blunt` 可用職業包括戰士、十字軍、暗影騎士、牧師、遊俠、薩滿、德魯伊、武僧、天騎士、巫師、術士與恩路者 / 幻術師；吟遊詩人與盜賊沒有列在這個熟練類型裡。Bow 則只列具備弓術熟練度的六職：遊俠、戰士、十字軍、暗影騎士、盜賊、吟遊詩人；多數非 Ranger 只是把高洞 bow 當 ranged slot / stat stick。

`Charm`（護石 / 查姆）則是全職業都能使用的副手 / 工具位。FC2 Unique 表中 `Fallen Construct Charm` 明確列為 `Socketed (2～5)`，可當已知 5-slot 代表；若看到低等 5-slot Charm，先保留。站內 FC2 Legendary 表另有 Lv10 `Naiko's Cinder Charm` 顯示 `Socketed (4～6)`，因此 Charm 上限先採保守寫法：5 洞必留，6 洞資料請以現版本遊戲 tooltip 複核後再下定論。

實務順序可以這樣抓：

1. 先準備兩把低等高 socket 單手鈍器，讓 14 職業都能在雙持後吃到 `1h + 1h` 的 `+120%`。
2. 如果撿到低等 6-socket 雙手鈍器或 `Stave`，先進銀行；單件 6 顆 `Thex` 就能達到 `+120%`，戰士還可評估雙持雙手的極限路線。
3. 六個 Archery 職業若撿到 6-slot bow，可作副手 / sub 的 `+60%` XP 工具；`Demetrium's Ballista` 是代表例，但不是每把都 6 sockets。
4. 不能用 bow 的職業，或想用副手補工具屬性的角色，改看高洞 `Charm`；5 slots 先按 `+50%` XP 計算。
5. Lv1 或雙持條件未滿前，副手可以先用盾牌補生存，但不要把 `Thex` 塞進盾牌期待經驗加成。
6. 鑲嵌前先檢查成品 tooltip 的 required level、屬性需求與欄位效果。`Thex` 的 `Required Lv 45` 是 rune / 升級表資訊；目前實測顯示它插入 Lv5 單手鈍器後，不會把該裝備需求等級提高到 45。
