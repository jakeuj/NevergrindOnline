...
- Document `Thex` XP heirlooms by equipment slots: right hand, left hand, and sub slot. `1h + 1h + bow` is 18 `Thex` for `+180%`; Warrior `2h + 2h + bow` is also 18 `Thex` but reaches `+300%` because two-hand weapons are `+20%` per slot. Bow is only for Ranger, Warrior, Crusader, Shadow Knight, Rogue, and Bard. `Charm` is usable by all 14 classes; 5-slot `Charm` is a must-keep tool, while possible 6-slot `Charm` data should be tooltip-verified.
...
- In `blacksmith-crafting-recipe-research.md`, keep the Mythical substrate keeper table aligned with highest recipe slot needs: `twoHandSlashers` Lv72 / 6 slots, `twoHandBlunts` Lv71 / 6, `oneHandBlunts` Lv70 / 6, `Focus` Lv65 / 6, `Staves` Lv62 / 6, `PIERCERS` Lv67 / 5, `oneHandSlashers` Lv62 / 5, `bows` Lv63 / 5, `shields` Lv62 / 4, `chests` Lv58 / 4, and `helms` Lv58 / 3. Never regress to the old mistakes that `PIERCERS` or `oneHandSlashers` require 6 slots, or that `oneHandBlunts` require only 5 slots.
...
- For `fc2-general-reference.md`, use separate page-specific quality arrays for its bundled source pages, for example `FC2_CHART_TERMINOLOGY_PATTERNS` and `FC2_UNIMON_TERMINOLOGY_PATTERNS`, because `chart.html`, `unimon.html`, `loot.html`, and `gambling.html` have different failure modes despite sharing one output Markdown file.
