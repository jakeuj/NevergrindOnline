...
- In rune docs, keep `Charm` / 護身符 distinct from `Amulet` / 項鍊. For `Thex`, current tooltip evidence says one-hand weapons and `Charm` give XP, two-hand weapons give doubled XP, and shields use the armor effect `+15% 最大法力`; never present shields as an XP source.
